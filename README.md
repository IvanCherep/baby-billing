...
<summary>Общая схема проекта</summary>
	<p>CDR-севрис передает файлы в BRT-сервис через брокер сообщений, однако запросы на изменение тарифа или пополнение счета посылает напрямую. Также вместе с проектом поднимается pgAdmin.</p>
